    ...
    VALUE obj;
{
	...
	if ((ret = db_open(name, dbst->type, flags, mode, dbenvp,
			   dbst->dbinfo, &dbp)) != 0) {
	    if (bdb_errcall) {
		bdb_errcall = 0;
...
static VALUE
bdb__txn__close(VALUE obj, VALUE commit)
{
    bdb_DB *dbst, *dbst1;
...
void bdb_init_common()
{
    ...
    rb_define_private_method(bdb_cCommon, "__txn_close__", bdb__txn__close, 1);
