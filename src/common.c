    ...
    VALUE obj, *argv;
{
	...
	st = Data_Make_Struct(rb_cData, struct bdb_eiv, 0, free, biv);
	biv->bdb = res;
	biv->dbst = dbst;
	rb_ary_push(bdb_internal_ary, st);
    }
    return res;
}
...
#endif
    bdb_internal_ary = rb_ary_new();
    rb_set_end_proc(bdb_finalize, bdb_internal_ary);
